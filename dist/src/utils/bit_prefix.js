...
class BitPrefix {
     ...
     */
    static of(fullBytes, prefixLength) {
        if (prefixLength > fullBytes.length * BitPrefix.BYTE_SIZE) {
            throw new Error('Invalid length of bytes array');
        }
        const prefixBytesLength = Math.floor((prefixLength + BitPrefix.BYTE_SIZE - 1) / BitPrefix.BYTE_SIZE);
    ...
    toString() {
        ...
        return this.prefix.reduce((a, b, idx) => a += this.toBinaryStr(b, idx), '0b');
    }
    /**
     * Creates a bit mask for the last byte of the prefix.
     */
    static bitMask(prefixLength) {
...
exports.BitPrefix = BitPrefix;
BitPrefix.BYTE_SIZE = 8;
